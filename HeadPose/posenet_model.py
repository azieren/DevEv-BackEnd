...
class PoseHighResolutionNet(nn.Module):
    ...
    def forward_feature(self, x):
        ...
        B, D, _, _ = x_list[-1].size()
        x = self.final_layer(y_list[0])

        feature = x_list[-1].view((B, D, -1)).mean(-1)
        
        coords, _ = get_max_preds(x.cpu().numpy())
        coords = torch.from_numpy(coords).to(x.device)
        coords[:,:,0] /= H
        coords[:,:,1] /= W
        coords = coords.view(B, -1)
        feature = torch.cat([feature, coords] , dim = -1)
        #feature = coords
        return feature
