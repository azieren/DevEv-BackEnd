...
def parse_opt():
    ...
    parser.add_argument(
        '--data', 
        default="data_configs/devev.yaml",
    ...
    parser.add_argument(
        '-w', '--weights', 
        default="outputs/training/output/training/best_model.pth",
...
def get_box_from_view(box, view, h, w):
    x1, y1, x2, y2 = box
    return np.array([x1 + w*(view%2), y1 + h*(view//2), x2, y2])

def main_inference_toy(args):
    ...
    while(cap.isOpened()):
        ...
        if ret:
            ...
            for i in range(8):
                ...
                frame_v = resize(frame_v, RESIZE_TO, square=args['square_img'])
                image = frame.copy()
