...
class CustomDataset(Dataset):
    def __init__(
        ...
        self.all_annot_paths = glob.glob(os.path.join(self.labels_path, '*.json'))
    ...
    def read_and_clean(self):
        ...
        for image_name in tqdm(self.all_images, total=len(self.all_images)):
            possible_xml_name = os.path.join(self.labels_path, os.path.splitext(image_name)[0]+'.json')
    ...
    def load_image_and_labels(self, index):
        ...
        annot_filename = os.path.splitext(image_name)[0] + '.json'
        ...
        with open(annot_file_path, "rb") as f:
            info = json.load(f)    
        for data in info:
            labels.append(self.classes.index(data["label"]))
